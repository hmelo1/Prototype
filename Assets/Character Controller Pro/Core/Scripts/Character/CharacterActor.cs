...
namespace Lightbug.CharacterControllerPro.Core
{
    ...
    public class CharacterActor : PhysicsActor
    {
        ...
        void ProbeGround(float dt)
        {
            ...
            //Vector3 overlapTestPosition = position + collisionInfo.displacement;
            bool overlapCheck = characterCollisions.CheckOverlap(
                position,
                StepOffset,
                in overlapFilter
            );
            ...
            if (badStepDetected)
            {
                if (preventBadSteps)
                {
                    if (WasGrounded)
                    {
                        ...
                        // If the body is 2D, re-define velocity.
                        // This eliminates a small "velocity leak" caused by the collide and slide algorithm.
                        if (Is2D)
