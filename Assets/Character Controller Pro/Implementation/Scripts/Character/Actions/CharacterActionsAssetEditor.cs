        ...
        public override void OnInspectorGUI()
        {
            ...
            {       
                ...
                if (result)
                {
                    ...
                    string[] characterActionsResults = AssetDatabase.FindAssets(CharacterActionsFileName);

                    if (characterActionsResults.Length != 0)
                    {
                        string assetGUID = characterActionsResults[0];
                        characterActionsPath = AssetDatabase.GUIDToAssetPath(assetGUID);
                    }

                    string[] templateResults = AssetDatabase.FindAssets("template-character-actions");

                    if (templateResults.Length != 0)
                    {
                        string assetGUID = templateResults[0];
                        templatePath = AssetDatabase.GUIDToAssetPath(assetGUID);
                    }
